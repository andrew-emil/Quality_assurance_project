			...
			<Form method="POST">
				...
				<div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-center">
					...
					<div className="grow">
						...
						{formErrors?.phone && (
							<p className="mt-2 rounded-md bg-red-100 p-2 text-xs text-red-700" id="phone-error">
								{formErrors.phone}
							</p>
						)}
					</div>
				...
				<div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-center relative">
					...
					<div className="grow">
						...
						{error && addressStatus === "error" &&
							<p className="mt-2 rounded-md bg-red-100 p-2 text-xs text-red-700" id="address-error">{error}</p>
						}
					</div>
				...
				<div>
					...
					<Button disabled={isSubmitting || isLoadingAddress} type="primary">
						{isSubmitting ? "Placing order...." :
							`Order now for `}
						{!isSubmitting && <span id="total-price">{formatCurrency(totalPrice)}</span>}
					</Button>
				</div>
			</Form>
		</div>
